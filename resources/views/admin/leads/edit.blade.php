    ...
    <div>
        <div class="max-w-4xl mx-auto py-10 sm:px-6 lg:px-8">
            ...
            <div class="mt-5 md:mt-0 md:col-span-2">
                <form method="post" action="{{ route('admin.leads.update', $lead->id_client_lead) }}">
                    @csrf
                    @method('PUT')
                    <div class="shadow overflow-hidden sm:rounded-md">
                        ...
                        <div class="px-4 py-3 bg-white sm:p-6">
                            ...
                            <input type="email" name="email" id="email" class="form-input rounded-md shadow-sm mt-1 block w-full" value="{{ old('email', $lead->email) }}" />
                            @error('email')
                            <p class="text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="px-4 py-3 bg-white sm:p-6">
                            ...
                            <input type="text" name="phone" id="phone" class="form-input rounded-md shadow-sm mt-1 block w-full" value="{{ old('phone', $lead->phone) }}" />
                            @error('phone')
                            <p class="text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="px-4 py-3 bg-white sm:p-6">
                            ...
                            <input type="text" name="courses_name" id="courses_name" class="form-input rounded-md shadow-sm mt-1 block w-full" value="{{ old('courses_name', $lead->courses_name) }}" />
                            @error('courses_name')
                            <p class="text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="px-4 py-3 bg-white sm:p-6">
                            ...
                            <select name="sellers" id="sellers" class="form-select block rounded-md shadow-sm mt-1 block w-full">
                                @foreach($sellers as $seller)
                                    <option value="{{ $seller->id }}" {{ in_array($seller->id, old('sellers', $lead->users->pluck('id')->toArray())) ? ' selected' : '' }}>
                        ...
                        <div class="px-4 py-3 bg-white sm:p-6">
                            ...
                            <select name="channels" id="channels" class="form-select block rounded-md shadow-sm mt-1 block w-full" required="required">
                                {{--@foreach($channels as $id => $channel)
                                    <option value="{{ $id }}" @if ($lead->channel->id_channel === $id) selected @endif>
                                        {{ $channel }}
                                    </option>
                                @endforeach--}}
                                    @foreach($channels as $channel)
                                        <option value="{{ $channel->id_channel }}" @if (old('channels', $lead->channel->id_channel) === $channel->id_channel) selected @endif>
